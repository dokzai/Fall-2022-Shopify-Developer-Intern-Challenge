...
**So you can skip the setup**: I've left a link to the repl.it demo in my application, where you can feel free to run the automated testing, and play around with the code or use postman! I recommended running `npm run test` to test and see the basic functionality. **Created inventory items gets automatically assigned to a warehouse, if there is no warehouse available it will not create the item**
